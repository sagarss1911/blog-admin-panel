...
export class CustomMultiSelectComponent implements OnInit {
  ...
  onClickAddNew() {
    ...
    var tempSubObj: Subscription = this.modalService.onHide.subscribe(() => {
      if (this.modalRef.content.decision == 'done') {
        ...
        let findI = findIndex(this.value, (v) => {
          return v[this.optionValue] == this.modalRef.content.dialogResult;
        });
        if (findI == -1) {
          this.value.push(t);
  ...
  applyFilter() {
    let tempList = !this.filter_text
      ? this.option_list
      : this.option_list.filter((ol) => {
          return (
            ol[this.optionName]
              .toLowerCase()
  ...
  removeSelection(val) {
    if (val[this.optionValue] && val[this.optionName] && this.value) {
      let findI = findIndex(this.value, (v) => {
        return v[this.optionValue] == val[this.optionValue];
      });
      if (findI != -1) {
    ...
    if (Array.isArray(this.value)) {
      this.value.forEach((i) => {
        if (i[this.optionName]) {
          let fData = this.value_list.filter(
            (it) => it[this.optionValue] == i[this.optionValue]
          );
  ...
  validate(item) {
    let fData = this.value.filter(
      (it) => it[this.optionValue] == item[this.optionValue]
    );
  ...
  ngOnChanges(changeRecord: any) {
    if (
      !this.filter_text &&
      this.temp_option_list.length != this.option_list.length
    ) {
      this.temp_option_list = JSON.parse(JSON.stringify(this.option_list));
    }
    ...
    if (
      changeRecord &&
      changeRecord.value &&
      changeRecord.value.currentValue &&
      changeRecord.value.currentValue.length == 0
    ) {
      this.value_list = [];
    } else if (
      changeRecord &&
  ...
  onChangeSelection(option, index) {
    if (option[this.optionValue] && option[this.optionName] && this.value) {
      let findI = findIndex(this.value, (v) => {
        return v[this.optionValue] == option[this.optionValue];
      });
      if (findI == -1) {
  ...
  sendData() {
    let newValue = Array.isArray(this.value)
